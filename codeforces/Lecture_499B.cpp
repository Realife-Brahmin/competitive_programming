...
int main(){
	int n,m,x;cin>>n>>m;mstrstr M;string s1,s2;
	itr(i,0,m){cin>>s1>>s2;M.insert(mp(s1,s2));}
	mstrstr::iterator itr;
	itr0{
		cin>>s1;itr=M.find(s1);s2=itr->second;
		if(s1.length()>s2.length()){cout<<s2<<" ";}
		else{cout<<s1<<" ";}
	}
