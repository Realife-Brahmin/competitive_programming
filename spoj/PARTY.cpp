...
int main(){
	int s,n,w,v;
	while(1){
		int weight=0;cin>>s>>n;if(s==0&&n==0){return 0;}int A[n][s+1];vpi B;itr0{cin>>w>>v;B.pb(mp(w,v));}sort(B.be,B.en,&cmp);
			itr0{
				itr(we,0,s+1){
					if(we==0){A[i][we]=0;}
					else{
			...
			int i=n-1,we=s;
			while(i!=-1){
				int x=A[i][we],xl=A[i][we-1];
				if(xl==x){we--;}
				else{weight=we;i=-1;continue;}
			}
		cout<<weight<<" "<<A[n-1][s]<<"\n";
	}	
}
